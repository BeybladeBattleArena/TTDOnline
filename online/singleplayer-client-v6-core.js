...
async function refreshAll() {
  await Promise.all([refreshSnapshot(false), refreshSocial(), refreshProgression()]);
  sendSnapshot();
}
...
function friendStatusClass(status) { return status === 'accepted' ? 'friendAccepted' : status === 'incoming' ? 'friendIncoming' : 'friendOutgoing'; }

function attachLongPress(row, friend) {
  let timer = null; let startX = 0; let startY = 0; let opened = false;
...
async function renderFriendsModal() {
  if (!social || !snapshot) await Promise.all([refreshSocial(), refreshSnapshot(false)]);
  ...
  el('v6SaveName')?.addEventListener('click', async () => {
    try { social = (await call('setPublicDisplayName',{displayName:el('v6DisplayName').value})).social; await renderFriendsModal(); }
  ...
  el('v6SharedDie')?.addEventListener('change', async (event) => {
    try { social = (await call('setSharedDie',{instanceId:event.target.value || null})).social; await renderFriendsModal(); }
    catch (err) { alert(humanize(err)); }
  });
  el('v6AddFriend')?.addEventListener('click', async () => {
    const button=el('v6AddFriend'); button.disabled=true;
    try { social=(await call('sendFriendRequest',{friendCode:el('v6FriendCodeInput').value})).social; await renderFriendsModal(); }
    catch(err){alert(humanize(err)); button.disabled=false;}
  });
  modalCard.querySelectorAll('[data-accept]').forEach((button)=>button.addEventListener('click',async()=>{ try{social=(await call('acceptFriendRequest',{uid:button.dataset.accept})).social; await refreshProgression(); await renderFriendsModal();}catch(err){alert(humanize(err));} }));
...
async function openFriendMenu(friend) {
  ...
  `);
  el('v6ViewProfile')?.addEventListener('click', async()=>{
    try { const p=(await call('getPublicProfile',{uid:friend.uid})).profile; showModal(`<h2>${esc(p.displayName)}</h2><p>Friend Code: <strong>${esc(p.friendCode||'')}</strong></p>${p.sharedDie?`<p>Shared Die: <strong>${esc(titleKey(p.sharedDie.key))} · Class ${p.sharedDie.instance.cls}</strong></p>`:'<p>No Shared Die selected.</p>'}`); } catch(err){alert(humanize(err));}
  });
  el('v6ViewDeck')?.addEventListener('click', async()=>{
    try { const deck=(await call('getFriendDeck',{uid:friend.uid})).deck; hideModal(); postToGame({type:'ttd:friend-deck-view',deck}); }
    catch(err){alert(humanize(err));}
  });
  el('v6UseSupport')?.addEventListener('click', async()=>{
    try { social=(await call('selectSharedSupport',{uid:using?null:friend.uid})).social; await renderFriendsModal(); }
    catch(err){alert(humanize(err));}
  });
  el('v6RemoveFriend')?.addEventListener('click', async()=>{
    if(!confirm(`Remove ${friend.displayName} from your friends list?`)) return;
    try { social=(await call('removeFriend',{uid:friend.uid})).social; await renderFriendsModal(); }
    catch(err){alert(humanize(err));}
  });
}
async function renderProgressModal() {
  if (!progression) await refreshProgression();
  const a = progression.achievements || []; const d = progression.daily?.tasks || [];
  const row = (item, kind) => `<div class="progressRow ${item.claimed?'claimed':item.eligible?'ready':''}"><div><strong>${esc(item.name)}</strong><small>${esc(item.desc)}</small></div><div class="progressReward">${item.rewardPips} Pips <button data-${kind}="${esc(item.id)}" ${item.claimed||!item.eligible?'disabled':''}>${item.claimed?'Claimed':item.eligible?'Claim':'Locked'}</button></div></div>`;
  showModal(`<h2>Progress</h2><h3>Daily · ${esc(progression.daily.dayKey)}</h3>${d.map(x=>row(x,'daily')).join('')}<h3>Achievements</h3>${a.map(x=>row(x,'achievement')).join('')}`, true);
  modalCard.querySelectorAll('[data-achievement]').forEach((button)=>button.addEventListener('click',async()=>{ try{const data=await call('claimAchievement',{id:button.dataset.achievement});applyMutationPayload(data);await renderProgressModal();}catch(err){alert(humanize(err));} }));
  modalCard.querySelectorAll('[data-daily]').forEach((button)=>button.addEventListener('click',async()=>{ try{const data=await call('claimDaily',{id:button.dataset.daily});applyMutationPayload(data);await renderProgressModal();}catch(err){alert(humanize(err));} }));
}
...
giftBtn?.addEventListener('click', async()=>{
  ...
  try { const data=await call('redeemOnlineGiftCode',{code:giftInput.value}); applyMutationPayload(data); giftInput.value=''; const bits=[]; if(data.reward?.pips)bits.push(`${data.reward.pips} Pips`);if(data.reward?.astras)bits.push(`${data.reward.astras} Astras`);if(data.reward?.dice?.length)bits.push(`${data.reward.dice.length} die`);if(data.reward?.jewels?.length)bits.push(`${data.reward.jewels.length} jewel`); giftStatus.textContent=`Redeemed ${data.reward?.label||'gift'}${bits.length?`: ${bits.join(', ')}`:''}.`; await refreshProgression(); }
...
async function operation(name, data, resultType, requestId) {
  try {
    ...
    postToGame({ type:`${resultType}-error`, requestId, message:humanize(err) });
    throw err;
  }
}
...
window.addEventListener('message', async (event) => {
  ...
  // The old v4 deck client only knows three decks. Once a fourth or fifth deck exists,
  // capture its message before the v4 bubble listener and route it through v6 instead.
  if (m.type === 'ttd:deck-state-request' && Array.isArray(m.decks) && m.decks.length > 3) {
    event.stopImmediatePropagation();
    try {
      const data = await call('setDeckState',{decks:m.decks,activeDeckIdx:m.activeDeckIdx});
      await refreshSnapshot(false);
      postToGame({type:'ttd:v6-deck-state-result',requestId:m.requestId,snapshot});
    } catch(err) {
      postToGame({type:'ttd:v6-deck-state-error',requestId:m.requestId,message:humanize(err)});
    }
    return;
  }

  if (m.type === 'ttd:bridge-synced') {
    queueMicrotask(()=>{ if(!v6Synced) gameFrame.style.pointerEvents='none'; });
    return;
  }
  ...
  if (m.type === 'ttd:v6-refresh-request') { try{await refreshSnapshot(true);await refreshProgression();}catch(err){console.error(err);} return; }
  ...
  try {
    if (m.type === 'ttd:v6-socket-request') await operation('socketJewel',{dieId:m.dieId,jewelId:m.jewelId,slot:m.slot},'ttd:v6-socket-result',rid);
      ...
      applyMutationPayload(data); postToGame({type:'ttd:v6-run-finish-result',requestId:rid,...data}); await refreshProgression();
    }
  } catch (err) { console.error('V6 online operation failed.',m.type,err); }
}, true);
...
async function start() {
  ...
  onAuthStateChanged(auth,async(user)=>{
    ...
    catch(err){console.error('Could not load complete online account.',err); const status=el('status'); if(status){status.hidden=false;status.textContent=`Online account load failed: ${humanize(err)}`;status.dataset.kind='error';} gameFrame.style.pointerEvents='none';}
  });
}
start().catch((err)=>console.error('Single-player v6 client could not start.',err));
