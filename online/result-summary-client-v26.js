(() => {
  ...
  function ensureLegacyHideStyle(doc) {
    if (!doc?.head || doc.getElementById('ttd-result-summary-hide-v26')) return;
    const style = doc.createElement('style');
    style.id = 'ttd-result-summary-hide-v26';
    style.textContent = `
      #overlayGold, #overlayXpV21, #zSummaryXpV21 { display:none !important; }
      #zSummaryCard > .goldPill { display:none !important; }
      #ttdRunRewardsV26 { margin-top:10px; display:flex; flex-direction:column; gap:5px; text-align:center; }
      #ttdRunRewardsV26 .ttdRewardLine { font-size:13px; line-height:1.45; letter-spacing:.02em; }
      #ttdRunRewardsV26 .ttdRewardPips { color:var(--gold-glow,#f3d491); }
      #ttdRunRewardsV26 .ttdRewardExp { color:var(--astra-glow,#d4ecfa); }
      #ttdRunRewardsV26 .ttdRewardLevel { color:var(--parchment,#ece7da); font-size:12px; min-height:0; }
    `;
  ...
  function makeBlock(doc) {
    ...
    block.id = 'ttdRunRewardsV26';
    block.innerHTML = `
      <div class="ttdRewardLine ttdRewardPips" id="ttdRewardPipsV26">000 Pips banked!</div>
      <div class="ttdRewardLine ttdRewardExp" id="ttdRewardExpV26">000 EXP earned!</div>
      <div class="ttdRewardLine ttdRewardLevel" id="ttdRewardLevelV26"></div>
    `;
    return block;
  }
  ...
  function ensureRewardBlock() {
    ...
    let block = doc.getElementById('ttdRunRewardsV26');
