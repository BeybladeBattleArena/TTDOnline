(() => {
  ...
  function isElectrocutionStatusActive(e) {
  ...
  function makeHammerReticle(def) {
  ...
  function tickTargeting() {
    ...
    if (t >= targeting.deadline) { if(targeting.kind==='zetsascauldron') commitZetsa(); else if(targeting.kind==='hammerthundergod') commitHammerThunderGod(); else commitGaia(); }
  }
  ...
  window.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && targeting) { event.preventDefault(); const name=targeting.kind==='zetsascauldron'?"Zetsa's Cauldron":targeting.kind==='hammerthundergod'?'Hammer of Thunder God':'Gaia Crash'; clearTargeting(); toast(`${name} targeting canceled.`); }
  });
