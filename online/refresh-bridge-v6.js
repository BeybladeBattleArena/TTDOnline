...
/* ================= HG-A1 rapid-fire handguns / Bullet Halo V30 =================
   Kept in the existing isolated bridge chain so it executes inside the transformed v33
   game scope without eval, without touching Soul Saber hooks, and without changing loader order.
*/
const HGA1_KEY = 'hga1';
...
function hga1PairCenter(idx) {
  const col = idx % 5;
  return cw * (0.30 + col * 0.10);
}
function hga1NormalGunOrigin(idx, side, aim, kick) {
  const pairX = hga1PairCenter(idx);
  const baseX = pairX + (side===0 ? -22 : 22);
...
function hga1QueueShot(idx, die, side, target, dmg, akimbo) {
  if (!target?.alive) return;
  const {h} = ensureHga1State(die);
  const tp = enemyRenderPos(target);
  const currentAim = side===0 ? h.aimLeft : h.aimRight;
  const currentKick = side===0 ? h.kickLeft : h.kickRight;
  const base = hga1NormalGunOrigin(idx,side,currentAim,currentKick);
...
function hga1FireNormal(idx, die, d) {
  ...
  if (wantsAkimbo && h.leftAmmo>0 && h.rightAmmo>0) {
    ...
    hga1QueueShot(idx,die,0,target,baseDmg*T.akimboMult,true);
    hga1QueueShot(idx,die,1,target,baseDmg*T.akimboMult,true);
    return;
  }
  let side = h.nextGun;
  ...
  hga1QueueShot(idx,die,side,target,baseDmg,false);
  ...
  for(const rec of hga1Dice){
    ...
    if(h.phase==='normal' || h.phase==='reload'){
      ...
      for(const side of [0,1]){
        const baseX=hga1PairCenter(idx)+(side===0?-22:22);
        const aim=Math.atan2(tp.y-ch*.895,tp.x-baseX);
        if(side===0)h.aimLeft=aim;else h.aimRight=aim;
        const o=hga1NormalGunOrigin(idx,side,aim,side===0?h.kickLeft:h.kickRight);
    ...
    if(h.phase==='haloWaitReload' || h.phase==='postHaloReloadIn' || h.phase==='postHaloReload' || h.phase==='postHaloReloadOut'){
      const a=-Math.PI/2,pairX=hga1PairCenter(idx);
      drawHga1Image(hga1TopImage,pairX-22,ch*.895,74,19,a,h.reloadOpacity);
      drawHga1Image(hga1TopImage,pairX+22,ch*.895,74,19,a,h.reloadOpacity);
    }
    const c=h.cast;
