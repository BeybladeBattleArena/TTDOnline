(() => {
  'use strict';
  if(window.__TTD_ARCADE_MODE_SHELL_V1)return;
  ...
  const show=(name)=>{try{if(typeof showScreen==='function'){showScreen(name);return true;}}catch(_){}return false;};
  ...
  function installStyles(){
    if(document.getElementById('ttdArcadeModeShellStyleV1'))return;
    const style=document.createElement('style');style.id='ttdArcadeModeShellStyleV1';style.textContent=`
      #modeScreen .modeBody{
        justify-content:flex-start!important;overflow-y:auto!important;overscroll-behavior:contain!important;
        -webkit-overflow-scrolling:touch!important;touch-action:pan-y!important;padding-bottom:max(24px,env(safe-area-inset-bottom))!important;
      }
      #${MOVING_SCREEN_ID} .modeBody,#${KOTH_SCREEN_ID} .modeBody{
        flex:1;display:flex;flex-direction:column;gap:12px;padding:16px 16px max(24px,env(safe-area-inset-bottom));
  ...
  function makeScreen(id,title,kind,intro){
    ...
    screen=document.createElement('div');screen.id=id;screen.className='screen ttdArcadeLandingScreenV1';
    ...
    screen.querySelector('.ttdArcadeLandingBackV1')?.addEventListener('click',()=>show('mode'));
  ...
  function openMoving(){ensureScreens();show(MOVING_SCREEN_ID.replace(/Screen$/,''));}
  function openKoth(){ensureScreens();show(KOTH_SCREEN_ID.replace(/Screen$/,''));}
  ...
  function beginOnlineRun(modeKey,mapKey,starter){
    ...
    if(!deckReady()){toast('Your deck needs all 5 dice filled');show('deck');return;}
  ...
  function presentMovingResult(box,state){
    ...
    document.getElementById('ttdRunRewardsV26')?.remove();
