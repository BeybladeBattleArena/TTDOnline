(() => {
  ...
  const STYLE_ID = 'ttd-russo-one-v23';
  const LINK_ID = 'ttd-russo-one-font-v23';
  const FONT_URL = 'https://fonts.googleapis.com/css2?family=Russo+One&display=swap';
  ...
  function installCanvasFont(win) {
    ...
    if (!proto || proto.__ttdRussoOneV23) return;
    ...
    try {
      ...
      Object.defineProperty(proto, '__ttdRussoOneV23', { value: true, configurable: false });
    } catch (err) {
  ...
  // Keep Zombie typography untouched through battle and its result screen. Returning Home restores
  // Russo One for the rest of the game without changing any of Zombie Mode's original font rules.
