(() => {
  ...
  function draw(){const lane=document.getElementById('laneWrap'),s=stage(),st=state();if(!lane||!s||!st)return;ensure(lane);const{W,H}=size(lane);ctx.clearRect(0,0,W,H);for(const d of s.constructionDecor||[]){if(d.kind==='rebar')drawRebar(ctx,d,st.cameraY,W,H);else if(d.kind==='safetyFence')drawFence(ctx,d,st.cameraY,W,H);else if(d.kind==='plywoodStack')drawStack(ctx,d,st.cameraY,W,H,false);else if(d.kind==='crateStack')drawStack(ctx,d,st.cameraY,W,H,true);else if(d.kind==='craneTower')drawCraneTower(ctx,d,st.cameraY,W,H);else if(d.kind==='craneBoom')drawCraneBoom(ctx,d,st.cameraY,W,H);} }
