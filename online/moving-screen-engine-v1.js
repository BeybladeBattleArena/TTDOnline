(() => {
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      #movingScreenScreen .msTitleWrap{min-width:0}#movingScreenScreen .msTitle{font:700 12px 'Cinzel',serif;color:#d4ecfa;letter-spacing:.07em;white-space:nowrap}#movingScreenScreen .msSubtitle{font:700 7px 'Space Mono',monospace;color:#70799b;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      #movingScreenScreen .msStats{margin-left:auto;display:flex;gap:6px;align-items:center}#movingScreenScreen .msPill{border:1px solid #2a3160;border-radius:14px;background:#11162a;padding:5px 8px;color:#ccd3e3;font:700 9px 'Space Mono',monospace;white-space:nowrap}#movingScreenScreen .msPill strong{color:#f3d491}#movingScreenScreen .msPill.moving{border-color:rgba(226,88,79,.55);color:#ffb0a9;box-shadow:0 0 10px rgba(226,88,79,.18)}
      #movingScreenScreen .msWorld{position:relative;flex:1;min-height:0;overflow:hidden;background:#050812}#movingScreenScreen .msWorld canvas{position:absolute;inset:0;width:100%;height:100%;display:block;touch-action:none}
      #movingScreenScreen .msActions{position:absolute;left:8px;right:8px;bottom:8px;display:flex;align-items:flex-end;gap:8px;z-index:8;pointer-events:none}#movingScreenScreen .msActions button{pointer-events:auto;min-height:44px;border-radius:12px;border:1px solid rgba(255,255,255,.18);padding:9px 13px;background:rgba(13,18,35,.9);color:#ece7da;font:700 10px 'Cinzel',serif;box-shadow:0 5px 16px rgba(0,0,0,.35)}#movingScreenScreen .msActions .msSummon{margin-left:auto;background:linear-gradient(180deg,#f3d491,#d9b26a);color:#0a0c14;border:0;min-width:116px}#movingScreenScreen .msActions button:disabled{opacity:.38}
      ...
      #movingScreenScreen .msToast{position:absolute;left:50%;top:12px;transform:translateX(-50%);z-index:10;pointer-events:none;opacity:0;transition:opacity .18s;background:rgba(5,8,18,.94);border:1px solid rgba(243,212,145,.36);border-radius:16px;padding:7px 12px;color:#f3d491;font:700 9px 'Space Mono',monospace;white-space:nowrap;max-width:86%;overflow:hidden;text-overflow:ellipsis}#movingScreenScreen .msToast.show{opacity:1}
      #movingScreenScreen .msMoveBanner{position:absolute;left:50%;top:13%;transform:translateX(-50%) scale(.95);z-index:9;pointer-events:none;opacity:0;color:#fff;font:700 clamp(24px,7vw,58px) 'Cinzel',serif;letter-spacing:.13em;text-shadow:0 3px 0 #821f26,0 0 26px rgba(255,70,70,.62);transition:opacity .18s,transform .18s}#movingScreenScreen .msMoveBanner.show{opacity:1;transform:translateX(-50%) scale(1)}
      #movingScreenScreen .msProgress{position:absolute;right:10px;top:62px;bottom:74px;width:28px;z-index:6;pointer-events:none;display:flex;justify-content:center}#movingScreenScreen .msProgressRail{position:absolute;top:8px;bottom:8px;width:3px;border-radius:3px;background:rgba(255,255,255,.13);box-shadow:0 0 10px rgba(80,170,255,.08)}#movingScreenScreen .msProgressFill{position:absolute;bottom:8px;width:3px;border-radius:3px;background:linear-gradient(0deg,#55d8ff,#ff58cc);height:0}#movingScreenScreen .msProgressDot{position:absolute;left:50%;width:12px;height:12px;margin-left:-6px;border-radius:50%;border:2px solid rgba(255,255,255,.7);background:#11162a;box-shadow:0 0 9px rgba(85,216,255,.3)}#movingScreenScreen .msProgressCamera{position:absolute;left:50%;width:20px;height:8px;margin-left:-10px;border-radius:7px;background:#f3d491;box-shadow:0 0 12px rgba(243,212,145,.65)}
      #movingScreenScreen .msResult{position:absolute;inset:0;z-index:14;display:none;align-items:center;justify-content:center;background:rgba(4,6,14,.8);backdrop-filter:blur(5px);padding:18px}#movingScreenScreen .msResult.show{display:flex}#movingScreenScreen .msResultCard{width:min(390px,94vw);padding:20px;border:1px solid #30385d;border-radius:16px;background:linear-gradient(160deg,#161d34,#0d1122);text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.5)}#movingScreenScreen .msResultCard h2{font:700 22px 'Cinzel',serif;color:#f3d491;margin:0 0 8px}#movingScreenScreen .msResultCard p{font-size:11px;color:#a8b0ca;line-height:1.55}#movingScreenScreen .msResultCard button{width:100%;margin-top:10px;padding:10px;border:0;border-radius:10px;background:linear-gradient(180deg,#f3d491,#d9b26a);font:700 12px 'Cinzel',serif;color:#0a0c14}
      ...
      @media(max-width:560px){#movingScreenScreen .msTitle{font-size:10px}#movingScreenScreen .msSubtitle{display:none}#movingScreenScreen .msStats .msPill:nth-child(2){display:none}#movingScreenScreen .msHint{max-width:54vw;font-size:8px}.modeCard.msModeCard{padding:13px}#movingScreenScreen .msActions button{padding:8px 10px}#movingScreenScreen .msProgress{right:5px}}
    `;
  ...
  function summonDie(){
    ...
    const slot=free[Math.floor(Math.random()*free.length)].i;placeAtZone(entity,zone.id,slot);runtime.entities.push(entity);runtime.playerEverSummoned=true;runtime.selectedId=entity.id;
  ...
  function hitRouteOption(entity,px,py){for(const o of playerRouteOptions(entity)){const n=nodeById(o.to),p=projectPoint(n.x,n.z,n.y);if(Math.hypot(px-p.x,py-p.y)<22)return o;}return null;}
  function hitSpot(px,py,zoneId=null){let best=null,bestD=Infinity;for(const zone of runtime.stage.zones){if(zoneId&&zone.id!==zoneId)continue;for(let i=0;i<zone.spots.length;i++){if(spotOwner(zone.id,i,null))continue;const p=projectPoint(zone.spots[i].x,zone.spots[i].z,zone.spots[i].y),d=Math.hypot(px-p.x,py-p.y);if(d<16&&d<bestD){best={zone,slotIdx:i};bestD=d;}}}return best;}
