(() => {
  'use strict';
  if(window.__TTD_ADVENTURE_PLATFORM_SELECTOR_V6)return;
  window.__TTD_ADVENTURE_PLATFORM_SELECTOR_V6=true;
  ...
  function selectBoardIndex(index,status,button){
    ...
    try{
      ...
      console.error('Navigator selector v6 failed to select board instance.',err);
  ...
  function buildSelector(){
    ...
    const liveIndices=Array.isArray(api.liveBoardIndices)?api.liveBoardIndices.slice():[];
    ...
    status.id='ttdNavigatorSelectorStatusV6';
    status.textContent=liveIndices.length?`Tap any illuminated summoned die. ${liveIndices.length} available.`:'No live summoned dice were reported by battle state.';
