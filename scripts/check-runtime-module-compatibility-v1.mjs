...
const missing=new Map(),missingWritable=new Map(),unknown=new Map(),movingScreenUnknown=[];
for(const file of runtimeFiles){
  const source=fs.readFileSync(file,'utf8'),manager=analyze(source,file);
  for(const ref of manager.globalScope.through){
    const name=ref.identifier.name;if(browserGlobals.has(name)||intentionallyOptionalLegacy.has(name))continue;
    const mode=refMode(ref);
    if(coreBindings.has(name)){
      if(!exposureLines.has(name)){if(!missing.has(name))missing.set(name,new Set());missing.get(name).add(`${file}:${mode}`);}
      else if((mode==='W'||mode==='RW')&&!exposureLines.get(name).includes('(value)=>')){if(!missingWritable.has(name))missingWritable.set(name,new Set());missingWritable.get(name).add(`${file}:${mode}`);}
    }else{
      if(!unknown.has(name))unknown.set(name,new Set());unknown.get(name).add(`${file}:${mode}`);
      if(file==='online/moving-screen-engine-v4.js')movingScreenUnknown.push(`${name}:${mode}`);
    }
  }
}
...
if(movingScreenUnknown.length){console.error(`Moving Screen v4 has unresolved runtime identifiers: ${movingScreenUnknown.sort().join(', ')}`);process.exit(1);}
...
console.log(`Runtime module compatibility verified across ${runtimeFiles.length} active/static-and-fetched modules; Moving Screen v4 has no unresolved externals.`);