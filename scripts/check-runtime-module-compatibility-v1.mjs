...
const coreManager = analyze(coreSource, 'random-dice-game-33.html#main-script');
...
const coreBindings = new Set(coreScope.variables.map((v)=>v.name));

const exposureLines = new Map();
for (const line of coreSource.split('\n')) {
  const match = line.match(/__ttdExposeCore\('([^']+)'/);
  if (match) exposureLines.set(match[1], line);
}

const missing = new Map();
const missingWritable = new Map();
  ...
  for (const ref of manager.globalScope.through) {
    ...
    if (coreBindings.has(name)) {
      if (!exposureLines.has(name)) {
        ...
        missing.get(name).add(`${file}:${mode}`);
      } else if ((mode==='W' || mode==='RW') && !exposureLines.get(name).includes('(value)=>')) {
        if (!missingWritable.has(name)) missingWritable.set(name,new Set());
        missingWritable.get(name).add(`${file}:${mode}`);
      }
    } else {
      if (!unknown.has(name)) unknown.set(name,new Set());
      unknown.get(name).add(`${file}:${mode}`);
    }
  }
}

if (unknown.size) {
  console.log('Runtime-module non-core externals (informational):');
  for (const [name, refs] of [...unknown].sort(([a],[b])=>a.localeCompare(b))) {
    console.log(`  ${name} <- ${[...refs].sort().join(', ')}`);
  }
}
if (missing.size || missingWritable.size) {
  console.error('Runtime compatibility facade is incomplete for code that actually executes:');
  for (const [name, refs] of [...missing].sort(([a],[b])=>a.localeCompare(b))) {
    console.error(`  missing ${name} <- ${[...refs].sort().join(', ')}`);
  }
  for (const [name, refs] of [...missingWritable].sort(([a],[b])=>a.localeCompare(b))) {
    console.error(`  getter-only but written ${name} <- ${[...refs].sort().join(', ')}`);
  }
  process.exit(1);
}
