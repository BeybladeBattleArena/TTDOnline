...
// Canonical font support for the native result rows.
...
// The result tally is now owned by the actual game source, not an outer client patch.
game=mustReplace(game,
...
const newCampaign=`  function campaignComplete(){\n    if(!state || state.__ttdOutcomeCommitted) return;\n    state.running = false;\n    state.__ttdEndRewardMeta = captureNativeRewardMeta();\n    const goldEarned = Math.round(state.wave*10 + state.kills*1.2 + (state.coinGold||0) + (state.typhoonDefeated?150:0));\n    account.gold += goldEarned;\n    let bonusChest = false;\n    if(state.typhoonDefeated && state.adventureDiffKey){\n      awardChest(state.adventureDiffKey);\n      if(Math.random() < getBonusChestChance()){ awardChest(state.adventureDiffKey); bonusChest = true; }\n    }\n    saveAccount();\n    document.getElementById('overlayTitle').textContent = state.typhoonDefeated ? 'Typhoon Has Fallen' : 'Stage Cleared!';\n    document.getElementById('overlayText').textContent = state.typhoonDefeated\n      ? ('Al Hata is cleared! ' + (bonusChest ? 'Two Frozen Island Chests have' : 'A Frozen Island Chest has') + ' been added to your inventory.')\n      : (state.adventureStage.name + ' complete.');\n    document.getElementById('overlayStats').textContent = \`\${state.kills} kills · \${state.coinGold||0} Pips collected\`;\n    renderNativeTallies({pips:goldEarned,xp:null,meta:state.__ttdEndRewardMeta});\n    overlay.classList.remove('show');\n    nativeOutcome('clear',()=>{\n      revealAdventureNative();\n      if(state.typhoonDefeated && state.adventureDiffKey)setTimeout(()=>showRewardPopup(state.adventureDiffKey),450);\n    });\n  }`;
...
// The presentation module keeps MISSION/START/countdown/outcome visuals, but it no longer wraps
// end-of-run functions. Canonical HTML owns exactly-once outcome dispatch.
...
// Stop loading the two stale result layers. Their jobs now live in the canonical game source.
...
// Stop the continuous-world bridge from wrapping campaignComplete/endMatch for reward metadata.
...
// The run bridge only forwards the authoritative finish result into the canonical result renderer.
const runUiPath='online/run-ui-bridge-v21.js';
let runUi=read(runUiPath);
runUi=mustReplaceRe(runUi,/  function expText\(m\)\{[\s\S]*?\n\n  function requiredReplace\(source,needle,replacement,label\)\{/m,
`  window.addEventListener('message',(event)=>{\n    if(event.origin!==ORIGIN||event.source!==window.parent)return;\n    const m=event.data||{};if(m.type!=='ttd:v6-run-finish-result')return;\n    window.__TTD_APPLY_VERIFIED_RUN_RESULT_V35?.(m);\n    if(Number(m.chestCount||0)>0){const text=document.getElementById('overlayText');if(text)text.textContent=\`Al Hata is cleared! \${Number(m.chestCount)===1?'A Frozen Island Chest has':\`${Number(m.chestCount)} Frozen Island Chests have\`} been added to your inventory.\`;}\n  });\n\n  function requiredReplace(source,needle,replacement,label){`,
  'run bridge canonical result forwarding');
write(runUiPath,runUi);

// Load the rebuilt PCM fail cue instead of the MP3 decoder path.
...
// Bump the canonical monolith URL so existing clients cannot keep force-cached v34.
