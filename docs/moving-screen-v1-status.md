...
The repository validators guard the active stage/engine, objective constants, connectivity, runtime compatibility, loader order, numeric range returns, camera-band summoning, and topological AI planning. The exact final feature-branch head must pass the complete GitHub Verify workflow before merge/deploy.
