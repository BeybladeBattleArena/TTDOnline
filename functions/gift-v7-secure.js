...
exports.redeemOnlineGiftCode = onCall({region:REGION,timeoutSeconds:30},async(request)=>{
  ...
  await db.runTransaction(async(tx)=>{
    ...
    summary={label:String(codeData.label||'Gift Code').slice(0,80),pips:reward.pips,astras:reward.astras,dice:outDice,jewels:outJewels,cards:{lesser:Number(reward.cards.lesser||0),master:Number(reward.cards.master||0)}};
  });
